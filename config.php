...
function mercadopago_generate_pix($order_id, $amount, $client_name, $client_email, $order_expiration)
{
	...
	if (!$client_email) {
		$client_email = 'megaevolute@gmail.com';
	}
...
function greenpag_generate_pix($order_id, $amount, $client_name, $customer_cpf)
{
	global $_settings;
	global $conn;
	$client_id = $_settings->info('greenpag_client_key');
	$client_secret = $_settings->info('greenpag_client_secret');
	$curl = curl_init();
    $data = [
        'amount' => $amount,
        'customer' => [
        ...
        'callback_url' => BASE_URL . 'webhook.php?notify=greenpag'
    ];
	curl_setopt_array($curl, array(
		CURLOPT_URL => 'https://greenpag.com/api/v1/payments',
		CURLOPT_RETURNTRANSFER => true,
		CURLOPT_ENCODING => '',
		CURLOPT_MAXREDIRS => 10,
		CURLOPT_TIMEOUT => 0,
	...
	$pix_qrcode = $response['data']['qr_code_image'];
	$txid = $response['data']['transaction_id'];
	$payment_method = 'GreenPag';
	$sql = 'UPDATE order_list' . "\r\n" . 'SET payment_method = \'' . $payment_method . '\', pix_code = \'' . $pix_code . '\', pix_qrcode = \'' . $pix_qrcode . '\', txid = \'' . $txid . '\'' . "\r\n" . 'WHERE id = ' . $order_id;
...
function drope_paggue_create_order($client_name, $order_item, $amount, $order_id)
{
	global $_settings;
	$curl = curl_init();
	$data = ['payer_name' => $client_name, 'amount' => (int) $amount, 'external_id' => $order_id, 'description' => $order_item];
...
function paggue_generate_pix($oid, $total_amount, $customer_name, $customer_email, $order_expiration)
{
	...
	$pix_code = drope_paggue_create_order($order_user, $order_item, $order_amount, $order_id);
...
function corrigir_duplicidade($oid, $conn)
{
	// Obtendo dados do pedido
	$stmt = $conn->prepare('SELECT o.code, o.order_numbers, p.qty_numbers, p.id AS pid 
                             FROM order_list o 
	...
	if ($result->num_rows > 0) {
		...
		return; // Se não encontrar o pedido, sai da função
	}

	// Obtendo números de pedidos válidos
	$stmt = $conn->prepare('SELECT order_numbers FROM order_list WHERE status <> 3 AND product_id = ?');
	...
	// Processando números
	$numeros_ja_vendidos = array_filter(explode(',', implode(',', $all_lucky_numbers)));
	$qty_numbers--;
	...
	$numbers = array_filter(explode(',', $order_numbers));

	// Função para calcular a partição
	function calcular_particao($count)
	{
		if ($count < 600) {
			return 1;
		} elseif ($count < 1000) {
			...
			// Para valores acima de 5000, use uma regra dinâmica
			return ceil($count / 500); // Aumenta a partição a cada 500 elementos
		}
	}

	// Definindo a partição usando a nova função
	$partiion = calcular_particao(count($numbers));

	$cotas = partition($numbers, $partiion);

	foreach ($cotas as $numbersBatch) {
		$stmt = $conn->prepare('SELECT * FROM order_list WHERE product_id = ? AND id <> ? AND status <> 3 AND order_numbers REGEXP ?');
		$stmt->bind_param('sss', $pid, $oid, implode('|', $numbersBatch));
		$stmt->execute();
		$find_orders_query = $stmt->get_result();

		if ($find_orders_query->num_rows > 0) {
			$count = 0;
			while ($number = $find_orders_query->fetch_assoc()) {
				$query = $conn->prepare('SELECT id FROM order_list WHERE product_id = ? AND order_numbers REGEXP ? AND status <> 3 AND id <> ?');
				$query->bind_param('sss', $pid, $number['order_numbers'], $oid);
				$query->execute();
				$resultQuery = $query->get_result();

				if ($resultQuery->num_rows > 0) {
					$new_number = $array_without_ja_vendidos[$count] ?? null; // Garante que exista um número disponível
					if ($new_number !== null) {
						$update = $conn->prepare('UPDATE order_list SET order_numbers = REPLACE(order_numbers, ?, ?) WHERE id = ?');
						$update->bind_param('ssi', $number['order_numbers'], $new_number, $oid);
						$update->execute();
					}
					$count++;
				}
...
if (!defined('DEV_NAME')) {
	define('DEV_NAME', 'MEGAEVOLUTE');
}

if (!defined('DEV_URL')) {
	define('DEV_URL', 'https:megaevolute.com.br');
}

if (!defined('SUPPORT_URL')) {
	define('SUPPORT_URL', 'https://megaevolute.com.br');
}
