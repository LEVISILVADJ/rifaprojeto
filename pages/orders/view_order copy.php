...
while ($row = $order_items->fetch_assoc()) {
	...
	if($status == '2'){
		if($row['json_roleta'] == NULL){
			...
			$valor_cotas = json_decode($row['valor_cotas'], true);
			$result = array_intersect($cotas_pedido, $cotas_premio);
			// Inicializa as roletas
			$roletas = [];
			$quant = floor($row['quantity'] / 10);
			$aleatorios = [];
			foreach($result as $resu){
				array_push($aleatorios, $resu);
			}
			$count_aleatorio = count($aleatorios);
			$num_aleatorio = rand(0, ($quant - $count_aleatorio));
			
			foreach (range(0, $quant - 1) as $i) {
				$ganhou = FALSE;
				$valor = 0;
				if($count_aleatorio > 0){
					if($i >= $num_aleatorio){
						$ganhou = TRUE;
						$count_aleatorio = $count_aleatorio - 1;
						$valor = $valor_cotas[$aleatorios[$count_aleatorio]];
		...
		if (is_array($dados_valor_cotas)) {
			...
			// log de erro ou tratamento alternativo
			error_log("dados_valor_cotas não é um array: " . print_r($dados_valor_cotas, true));
			$totalValores = 0;
		}
		// $valores = [];
		// foreach ($dados_valor_cotas as $key => $value) {
		...
		echo '
				...
				<script src="/assets/js/roletaV5.js"></script>
				<script>
				...
				$rainbowColors = [
					...
					"#8A2BE2", "#6A5ACD", "#4169E1", "#4682B4", "#32CD32", "#ADFF2F", "#FFD700",
					"#FF4500", "#DC143C", "#C71585", "#DA70D6", "#9932CC", "#6495ED", "#87CEEB",
					"#40E0D0", "#20B2AA", "#3CB371", "#7FFF00", "#FFFFE0", "#FFA07A", "#FA8072",
					"#E9967A", "#FF6347", "#FF69B4", "#FF1493"
				];
				$indexColor = 0;
				...
				while ($repeatCount < $minRepetitions) {
					foreach ($valores as $key => $value) {
						...
						echo "{ id: '$key', type: 'allin', color: '$currentColor', text: '$key', ikon: 'grade' },\n";
						$indexColor++;
						$repeatCount++;
						if($repeatCount == 4){
							echo '{ id: "1", type: "allin", color: "#dc0936", text: "Perdeu" },';
						}
						if($repeatCount == 8){
							echo '{ id: "2", type: "allin", color: "#dc0936", text: "Perdeu" },';
						}
